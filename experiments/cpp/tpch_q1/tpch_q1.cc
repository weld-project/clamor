...
int main(int argc, char** argv) {
  ...
  string weld_filename = "tpch_q1.weld";
  ...
  case Cluster::Role::DRIVER:
    {
      ...
      //string url = "http://weld-dsm-east.s3.amazonaws.com/lineitem_large";
      string url = "/home/ubuntu/clamor/baselines/tpch_data/lineitem_large";
      auto mapped_data = driver.map_url(url, /*local=*/true);
      ...
      void* res_load = driver.run_query(query_load, &args_load, nprocs * worker_ips.size());
      
      string query = Util::read("tpch_q1.weld");
      weld_vector args;
      ...
      weld_vector* res = (weld_vector*)driver.run_query(query, &args,
							nprocs * worker_ips.size());
      
      gettimeofday(&tend, 0);
      timersub(&tend, &tstart, &tdiff);

      cout << "Output: " << ((output*)(res->data))[0] << endl;
